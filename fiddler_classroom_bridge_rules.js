...
class Handlers
{
	...
	static function SanitizeClassroomResponse(body: String): String {
		...
		for (var match: System.Text.RegularExpressions.Match in matches) {
			...
			items.Append("{\"jxcdmc\":\"")
				.Append(JsonEscape(match.Groups[1].Value))
				.Append("\",\"jcdm2\":\"")
				.Append(JsonEscape(match.Groups[2].Value))
				.Append("\",\"rq\":\"")
				.Append(JsonEscape(match.Groups[3].Value))
				.Append("\"}");
		}
...
*/
    ...
    static function OnBeforeResponse(oSession: Session) {
        if (oSession.HostnameIs("jwweb.hebmu.edu.cn")
            && oSession.HTTPMethodIs("POST")
            && oSession.uriContains("/dev-api/appapi/appkxjs/classroom")) {
            FiddlerApplication.Log.LogFormat("classroom bridge matched: HTTP {0}", oSession.responseCode);
        }
