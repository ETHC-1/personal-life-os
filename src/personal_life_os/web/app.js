...
function renderClassroomTimeline() {
  ...
  const roomRows = rooms.map((room, index) => {
    ...
    const bars = groups.map(group => {
      ...
      const labelMarkup = long
        ? `<span class="bar-label"><span>${roomPeriods[group[0] - 1].split("-")[0]}</span><span>${roomPeriods[group[group.length - 1] - 1].split("-")[1]}</span></span>`
...
async function loadClassroomUsage() {
  try {
    ...
    if (state.classroomBuildings.length) {
      ...
      state.classroomBuildings.sort((a, b) => buildingPriority(a) - buildingPriority(b));
